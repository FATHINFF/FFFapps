...
def load_patients():
    if not os.path.exists(DATA_FILE):  # Jika file tidak ada, kembalikan list kosong
        return []
    
    try:
        with open(DATA_FILE, "r") as file:
            data = file.read().strip()  # Hapus spasi & newline
            return json.loads(data) if data else []  # Jika kosong, return []
    except json.JSONDecodeError:
        print("JSON rusak, membuat ulang file baru.")
        return []  # Jika file rusak, return []


# Fungsi untuk menyimpan data pasien ke JSON
# Fungsi untuk menyimpan data pasien ke JSON
def save_patients(patients):
    # Ubah semua float32 ke float biasa
    for patient in patients:
        for key, value in patient.items():
            if isinstance(value, np.float32):
                patient[key] = float(value)  # Convert ke float biasa
...
def show_cek_tumor():
    ...
    if uploaded_image is not None and patient_name:
        if st.button("Cek Hasil"):  # Tombol buat nge-trigger prediksi & penyimpanan
            ...
            st.success(result)  
            st.image(uploaded_image, caption="Gambar MRI yang Diupload", use_container_width=True)  # Baru gambar
