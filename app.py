...
def load_patients():
    ...
    try:
        with open(DATA_FILE, "r") as file:
            data = json.load(file)
            # Filter data yang tidak memiliki kunci 'name'
...
def show_cek_tumor():
    ...
    if uploaded_image is not None and patient_name:
        if st.button("Cek Hasil"):  # Tombol buat nge-trigger prediksi & penyimpanan
            ...
            st.success(result)  
            st.image(uploaded_image, caption="Gambar MRI yang Diupload", width=300)
...
if st.session_state.logged_in:
    ...
    menu = st.sidebar.radio("Pilih Menu", ("Informasi", "Cek Tumor", "Hasil Cek Tumor Otak"))
